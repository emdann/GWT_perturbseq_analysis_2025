...
def freedman_lane_one(df2, base_order_invpos, sign_vec, use_shet=True, robust=None):
    """
    Process one response gene using a shared sign-flip vector for the current iteration.

    df2: long table for a single response gene (rows = targets); must include:
         - 'ensg' (target ENSG IDs)
         - 'post_mean' (response)
         - 'shet' (optional covariate)
         - perturbation predictors (added later in fit_full_and_contrast)
    base_order_invpos: dict {ensg -> 0..M-1} giving index of each target in the
         common base target universe (built in run_single_iteration).
    sign_vec: np.ndarray shape (M,), entries in {+1, -1}; shared across all genes
         in this permutation iteration, indexed by base_order_invpos.
    use_shet: whether to include shet in reduced/full models (if it varies).
    robust: passed through to statsmodels .fit(cov_type=robust) if not None.

    Returns
    -------
    refit_on_permuted_residuals : callable
        When called, performs the sign-flip permutation, reconstructs y^π,
        fits the full model, and returns (beta_or_beta_sum, p_value).
    """
    # ----- Reduced model: y ~ const (+ shet if it varies); NO perturb betas here
    y = pd.to_numeric(df2["post_mean"], errors="coerce")
    y_z = zscore_series(y)
    if y_z is None:
        return None
    ...
    base_idx = [base_order_invpos.get(e, None) for e in ensg_subset]
    s = sign_vec[np.asarray(base_idx, dtype=int)].astype(float)

    # ----- apply sign flips and refit the full model
    def refit_on_permuted_residuals():
        r_pi = r * s
        y_pi = yhat_red.values + r_pi
        df2_pi = df2.copy()
        df2_pi["post_mean"] = y_pi
        beta_sum, p_sum, *_ = fit_full_and_contrast(
            df2_pi, use_shet=use_shet_here, robust=robust
        )
...
def run_single_iteration(
    ...
    # Generate sign-flip vector
    sign_vec = rng.choice([-1, 1], size=M).astype(int)
    ...
    for i, item in enumerate(refitters):
        if item is None:
            continue
        _, df2 = item
        refit = freedman_lane_one(df2, base_order_invpos, sign_vec, use_shet=use_shet, robust=robust)
        if refit is None:
            continue
        beta_pi, p_pi = refit()
        p_vec[i] = p_pi
